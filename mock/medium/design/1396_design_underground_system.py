...
class UndergroundSystem:
    ...
    def checkIn(self, id, checkin_station, checkin_time):
        self.checkin[id] = (checkin_station, checkin_time)
        
    def checkOut(self, id, checkout_station, checkout_time):
        checkin_station, checkin_time = self.checkin[id]
        key  = f"{checkin_station}-{checkout_station}"
        diff = checkout_time - checkin_time
        
        if key not in self.hs: self.hs[key] = (1, diff)
        else:
            count, elapsed_time = self.hs[key]
            self.hs[key] = (count + 1, elapsed_time + diff)
            
    def getAverageTime(self, checkin_station, checkout_station):
        key  = f"{checkin_station}-{checkout_station}"
        count, elapsed_time = self.hs[key]
